...
const AddRoleModal: React.FC<AddRoleModalProps> = ({
  ...
  const handleAddRole = async () => {
    try {
      if (!selectedRoleId) return; // Validate if a role is selected

      setLoading(true);
      setError(null);

      const response = await api(
        `/roles/assign_role_to_user/${userId}`,
        'POST',
        {},
      ...
      <div className="bg-white py-6 dark:border-strokedark dark:bg-[#1A222C]  rounded-lg p-5 shadow-lg w-96">
        ...
        >
          ...
          <div className="p-4 md:p-5 text-center">
            <button
              type="submit"
              className="text-white bg-green-600 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-5"
