...
interface DatePickerProps {
  value?: string; // Ensure value is expected as a string
  onChange?: (selectedDates: Date[], dateStr: string, instance: flatpickr.Instance) => void;
  readOnly?: boolean;
}

const DatePickerOne: React.FC<DatePickerProps> = ({ value, onChange, readOnly }) => {
  ...
  useEffect(() => {
    if (datePickerRef.current) {
      const fp = flatpickr(datePickerRef.current, {
        ...
        dateFormat: 'Y-m-d', // ISO format for compatibility
      ...
      // Set initial value
      if (value) {
        fp.setDate(value, false); // false prevents triggering onChange
      }
