...
const ModifyHomePaymentChannel = () => {
  const { id } = useParams<{ id: string }>();
  const location = useLocation();
  const { setLoading, setProgress } = useLoading();
  const { setAlert } = useAlert();

  const {paymentChannels} = useFetchPaymentChannels();
  ...
  useEffect(() => {
    if (location.state && location.state.payment) {
      ...
      setFormValues({
        account_payBill: payment.account_payBill,
        payment_channels_id: String(payment.payment_channels_id),
        account_number: payment.account_number,
      });
      setSelectedChannelName(payment.payment_channel.channel_name);
    }
  }, [location.state]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api(
        `/payment/update-home-payment-info/${id}`, 
        'PUT', 
        {},
        formValues,
        trackProgress,
  ...
  return (
    <DefaultLayout>
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <FormIncludes title="Update Payment Channels" />
        <div className="p-7">
...
export default ModifyHomePaymentChannel;
